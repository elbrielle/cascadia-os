  ...
  function initFav() {
    ...
    function selectFolder(key) {
      folders.forEach(function (f) {
        ...
        if (on && pathEl) {
          // Update only the LEAF crumb; the "Favorites" root + the per-skin
          // separators (DOS "\" in retro, "›" chevron in Aero) are CSS. Falls
          // back to the old flat-string path if the leaf span is ever missing.
          var leaf = pathEl.querySelector("[data-fav-leaf]");
          if (leaf) leaf.textContent = f.dataset.label || "";
          else pathEl.textContent = "C:\\Favorites\\" + (f.dataset.label || "");
        }
      });
