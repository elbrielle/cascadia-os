...
(function () {
  function init() {
    ...
    // inspiration, type, the Aero glass technique, and the wallpaper sources. Opened
        ...
        '<div class="os-dialog" role="dialog" aria-modal="true" aria-labelledby="credits-title">' +
          ...
          '<div class="os-dialog__body">' +
            ...
            '<dl class="credits">' +
              ...
              '<div><dt>Glass</dt><dd>The Aero skin’s glass and gloss are my own palette, built on techniques I learned from a few Frutiger Aero studies on Figma: Skyweaver’s "Aero OS :: redux", and solinus’s "Six MORE Glass Panes" and "Frutiger Aero Buttons". The wider research leaned on the <a href="https://frutigeraeroarchive.org/" target="_blank" rel="noopener">Frutiger Aero Archive</a>.</dd></div>' +
              '<div><dt>Wallpaper</dt><dd>Cascadia 95 runs on hand-tuned pixel scenes. The Aero skin swaps in two public-domain photographs: Mount Rainier over Bench Lake by Steve Redman for the National Park Service, and the aurora over Eielson Air Force Base, Alaska, by Joshua Strang for the U.S. Air Force. Both are U.S. government works.</dd></div>' +
            '</dl>' +
            '<div class="os-dialog__actions">' +
    ...
    window.CascadiaStartMenu.reboot = reboot; // hoisted; tweaks.js calls it on a skin switch
    ...
    /* ----- Reboot (interface skin switch) -------------------------------- */
    // Same CRT power-off as Shut Down, but instead of the manual "safe to turn
    // off" screen it auto-reloads: clear the boot gate so the splash replays,
    // then reload the CURRENT page (you land where you were, now reskinned).
    // tweaks.js calls this after persisting the new skin, so the reloaded page
    // resolves the chosen look and the full-screen splash hides the repaint —
    // making a 95<->Aero swap feel like a real OS restart (Elisha's idea).
    function reboot(targetUrl) {
      var url = targetUrl || (location.pathname + location.search);
      var ov = document.createElement("div");
      ov.className = "shutdown shutdown--restart";
      ov.setAttribute("role", "alertdialog");
      ov.setAttribute("aria-label", "Restarting");
      ov.innerHTML =
        '<div class="shutdown__crt" aria-hidden="true"></div>' +
        '<p class="shutdown__safe shutdown__restarting" hidden>Restarting…</p>';
      document.body.appendChild(ov);

      var reduce = false;
      try { reduce = matchMedia("(prefers-reduced-motion: reduce)").matches; } catch (_) {}

      // After the CRT collapse: flash "Restarting…", clear the gate, reload.
      setTimeout(function () {
        var r = ov.querySelector(".shutdown__restarting");
        if (r) r.hidden = false;
        try { sessionStorage.removeItem("cascadia-booted"); } catch (_) {}
        setTimeout(function () { window.location.href = url; }, reduce ? 60 : 340);
      }, reduce ? 80 : 720);
    }
  }
