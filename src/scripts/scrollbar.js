...
(function () {
  ...
  function init() {
    ...
    var targets = document.querySelectorAll("main .window .window__body, main .window .fav__grids, main .window--panes [data-scroll-pane]");
    Array.prototype.forEach.call(targets, function (el) {
      if (getComputedStyle(el).overflowY !== "auto") return;
      // On desktop a delegating window (favorites .fav__grids, or a `panes`
      // window's [data-scroll-pane]) hands its scroll to an inner pane, so skip
      // its body — it won't overflow (panes body is overflow:hidden) and we'd
      // otherwise draw a redundant hidden bar. On mobile the inner pane is
      // overflow:visible, so the body keeps its bar for the stacked/drill-in flow.
      if (el.classList.contains("window__body")) {
        var fg = el.querySelector(".fav__grids, [data-scroll-pane]");
  ...
  // A panes window can change WHICH element is the auto-overflow scroller at
  // runtime (the Art CD player: switching to a non-playable album flips the
  // tracklist [data-scroll-pane] from the scroller to flowing chrome, and the
  // .window__body becomes the block scroller — see art.css liner-mode :has()).
  // The ResizeObserver only re-places existing bars; it can't re-target. So a
  // component that mutates its scroll container dispatches this event to ask for
  // a full teardown + re-scan, keeping the themed bar on the right element.
  document.addEventListener("cascadia:rescroll", init);
